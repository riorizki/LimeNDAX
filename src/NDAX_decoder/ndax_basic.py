...
rec_columns = [
    ...
    'Current(A)', 'Capacity(Ah)','Energy(Wh)', 'Timestamp', 'Validated']
...
def main_validator(df, capacity_nom):
    ...
    if (df["Voltage"].min() < 2) and (df[df["Status"] == "SIM"].shape[0] == 0):
        ...
        return False
    if (df["Charge_Energy(Wh)"].min() < 0) and (df[df["Status"] == "SIM"].shape[0] == 0):
        logging.info(
            "Negative Energy error. Min energy is: " + str(df["Charge_Energy(Wh)"].min())
        )
        return False

    if (df["Charge_Capacity(Ah)"].min() < 0) and (df[df["Status"] == "SIM"].shape[0] == 0):
        logging.info(
            "Negative Capacity error. Min capacity is: " + str(df["Charge_Capacity(Ah)"].min())
        )
        return False

    if df["Charge_Capacity(Ah)"].max() > (1500 * capacity_nom):
        logging.info(
            "Max Capacity error. Max capacity is: " + str(df["Charge_Capacity(Ah)"].max())
        )
        return False
