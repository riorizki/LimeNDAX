...
def fabricate(df):
    ...
    if nan_value.any():
        warnings.warn("IMPORTANT:This ndax has missing data. The output from Code contains interpolated data for time, timeinstep, capacity and energy!")
    ...
    capacity = (df['Time'].diff().where(df['Time'].diff()>=0,df['Time'].diff().shift(1)))*abs(df['Current(A)'])/3600
...
def data_runInfo_ndc(file):
            ...
            for i in struct.iter_unpack(format, bytes[132:end_byte]):
                ...
                if Index != 0:
                    rec.append([Time/1000,
                                abs((Charge_Capacity/3600000) - (Discharge_Capacity/3600000)),
                                abs((Charge_Energy/3600000) - (Discharge_Energy/3600000)),
    ...
    df['Timestamp']=(df['Timestamp'].dt.tz_localize('Asia/Dhaka').dt.tz_convert('Asia/Kolkata')).dt.tz_localize(None)
...
def to_df(file, include_aux: bool = False,step_cyclic_id:bool=False):
    ...
    if all(i in os.listdir(r".\temdata") for i in ['data_runInfo.ndc', 'data_step.ndc']):
        ...
        data_df = data_df.merge(step_df, how='left', on='Step').reindex(columns=ndax_basic.rec_columns)
    ...
    # print("Time to get all bytes: ",ctime - stime )
        ...
        if('DCIR(mOhm)' not in df.keys()):
            ...
            df['DCIR(mOhm)']=-1.0
            df.loc[((df['prev_cur']==0)&(df['Current(A)']!=0)),'DCIR(mOhm)']=(abs((df['Voltage']-df['prev_vol']) /(df['Current(A)']-df['prev_cur']))*1000000).astype('float32')
...
def get_records(file, rename: bool = False, drop_cycle_if_gap : bool = False, include_aux: bool = False,step_cyclic_id: bool = False):
...
def get_stepxml(ndax_file): 
    ...
    df_1['Protect_Main_Volt_Upper']=df_1['Protect_Main_Volt_Upper'].astype('int').div(10000)
    df_1['Protect_Main_Volt_Lower']=df_1['Protect_Main_Volt_Lower'].astype('int').div(10000)
    df_1[df_1 == 0] = np.nan
...
def get_step(df):
    ...
    try:
        ...
        raise ValueError('Argument pushed into the function is neither an appropriate DataFrame nor a path to an ndax file')
    df=rename(df)
    print(df.columns)
    ...
    if('DCIR(mOhm)' not in df.keys()):
        ...
        df['prev_vol']=df['Voltage'].shift(periods=1)
        df['DCIR(mOhm)']=-1.0
        df.loc[((df['prev_cur']==0)&(df['Current(A)']!=0)),'DCIR(mOhm)']=(abs((df['Voltage']-df['prev_vol']).astype('float64') /(df['Current(A)']-df['prev_cur']).astype('float64'))*1000000)
        df.drop(columns=['prev_cur','prev_vol'],inplace=True)
...
def get_cycle(df):
    ...
    try:
        ...
        raise ValueError('Argument pushed into the function is not an appropiate DataFrame nor a path to an ndax file')
    ...
    if not((chg_temp in step_col) & (dchg_temp in step_col)):
        for col in step_col:
            ...
            if re.search('dchg',col,re.IGNORECASE):
                dchg_temp=col 
    lastcycle_counter = 0
    if (df[df['Cycle']==max(df['Cycle'])].Index.nunique() == df.groupby(['Cycle']).Index.nunique().mode()[0]):
        lastcycle_counter = 1
    ...
    for i in range(min(df['Cycle']),max(df['Cycle']+lastcycle_counter)):                         
        ...
        charging_time='{:02}:{:02}:{:02}'.format(int(chgtime//3600), int(chgtime%3600//60), int(chgtime%60))
        dchgtime = df_dchg.iloc[-1].at['Time']
        discharging_time='{:02}:{:02}:{:02}'.format(int(dchgtime//3600), int(dchgtime%3600//60), int(dchgtime%60))
...
def get_recipe(df):
    ...
    df_recipe=pd.DataFrame(zip(np.array(stepid).reshape(-1,1),
                            ...
                            np.array(cutoff_vol).reshape(-1,1)),columns=['Step','Cycle','Status','Voltage','Current(A)','Rest_time','Cutoff_current','Cutoff_voltage'])  
    ...
    for i in range(len(recipe_cycle)-1):
        ...
        if((df_temp1.reset_index(drop=True).shape)==(df_temp2.reset_index(drop=True).shape) and ndax_basic.df_diff(df_temp1.reset_index(drop=True),df_temp2.reset_index(drop=True))==1):
            continue
        else:
