...
def match_record_by_time(records, target_dt, tolerance=timedelta(seconds=1)):
    """Match record by time with tolerance."""
    best_row = None
    best_delta = None
    for row in records:
        dt = row.get("date") or row.get("datetime")
...
def find_step_by_type_and_duration(step_list, step_type, target_seconds):
    """Find step by type and duration."""
    best_step = None
    best_diff = float("inf")
    for step in step_list:
        typ = step.get("step_type", "").replace("_", " ").lower()
        if step_type.lower() in typ:
...
def check_step_5(step_list, aux, records):
    ...
    for step in step_list:
        ...
        if typ == "cc dchg":
            ...
            if duration is None:
                continue
            diff = abs(duration - 3600)  # 1 hour = 3600 seconds
            if diff < best_diff:
                best_diff = diff
                dchg_step = step
    ...
    for row in aux_win:
        ...
        if any(v is not None and v < 3.2 for v in string_vs):
            low_voltage_flag = True
            low_v_reason = f"StringV={min([v for v in string_vs if v is not None]):.2f}V"
            break
    if low_voltage_flag:
        results.append({
...
def check_step_6(step_list, aux_all):
    ...
    for step in step_list:
        ...
        if typ == "rest":
            ...
            if dur is None:
                continue
            diff = abs(dur - 2400)  # 40 minutes = 2400 seconds
            if diff < best_diff:
                best_diff = diff
                rest_step = step
        ...
        return [
            {
                ...
                "result": "NG",
                "detail": "No rest step",
                "reason": "Not found in step_list",
            }
        ]

    aux = get_aux_in_window(
        aux_all, rest_step.get("oneset_date", ""), rest_step.get("end_date", "")
    )
    results = []
...
def check_step_7(step_list, aux_all):
    ...
    for step in step_list:
        ...
        if typ == "cc chg":
            ...
            if duration is None:
                continue
            diff = abs(duration - 7200)  # 2 hours = 7200 seconds
            if diff < best_diff:
                best_diff = diff
                cc_chg_step = step
        ...
        return [
            {
                ...
                "result": "NG",
                "detail": "No CC Chg step found",
                "reason": "No step",
            }
        ]

    aux = get_aux_in_window(
        aux_all, cc_chg_step.get("oneset_date", ""), cc_chg_step.get("end_date", "")
    )
    results = []
...
def check_step_8(step_list, aux_all):
    """Check step 8: Final rest step with 10 seconds."""
    # Find last rest step with 10 seconds
    rest_step = None
    best_diff = float("inf")
    for step in reversed(step_list):
        ...
        if typ == "rest":
            ...
            if duration is None:
                continue
            diff = abs(duration - 10)  # 10 seconds
            if diff < best_diff:
                best_diff = diff
                rest_step = step
...
def run_all_checks(json_path):
    ...
    # Find steps using exact matching from individual checkers
    steps = {}
    
    # Step 1: Rest 10s
    best_step1 = None
    best_diff1 = float("inf")
    for step in step_list:
        if step.get("step_type", "").lower() == "rest" and step.get("step_time") in ("00:00:10", "10 seconds"):
            steps["step1"] = step
            break
        ...
        for step in step_list:
            ...
            if typ == "rest":
                ...
                if duration is not None:
                    diff = abs(duration - 10)
                    if diff < best_diff1:
                        best_diff1 = diff
                        best_step1 = step
        if best_step1:
            steps["step1"] = best_step1
    ...
    for step in step_list:
        ...
        if typ == "cccv chg":
            duration = parse_duration(step.get("step_time", ""))
            if duration is not None:
                diff = abs(duration - 10800)  # 3 hours = 10800 seconds
                if diff < best_diff2:
                    best_diff2 = diff
                    best_step2 = step
    if best_step2:
        steps["step2"] = best_step2
    ...
    for step in step_list:
        ...
        if typ == "rest":
            duration = parse_duration(step.get("step_time", ""))
            if duration is not None:
                diff = abs(duration - 1800)  # 30 minutes = 1800 seconds
                if diff < best_diff3:
                    best_diff3 = diff
                    best_step3 = step
    if best_step3:
        steps["step3"] = best_step3

    # Step 4: CC discharge 1h (for 180s check)
    best_step4 = None
        ...
        if typ == "cc dchg":
            ...
            if duration is not None:
                ...
                if diff < best_diff4:
                    ...
                    best_step4 = step
    if best_step4:
        steps["step4"] = best_step4

    # Step 5: CC discharge 1h (for full check)
    best_step5 = None
    best_diff5 = float("inf")
    for step in step_list:
        typ = step.get("step_type", "").replace("_", " ").lower()
        if typ == "cc dchg":
            duration = parse_duration(step.get("step_time", ""))
            if duration is not None:
                ...
                if diff < best_diff5:
                    ...
                    best_step5 = step
    if best_step5:
        steps["step5"] = best_step5

    # Step 6: Rest 40min
    best_step6 = None
    best_diff6 = float("inf")
    for step in step_list:
        typ = step.get("step_type", "").replace("_", " ").lower()
        if typ == "rest":
            duration = parse_duration(step.get("step_time", ""))
            if duration is not None:
                diff = abs(duration - 2400)  # 40 minutes = 2400 seconds
                if diff < best_diff6:
                    best_diff6 = diff
                    best_step6 = step
    if best_step6:
        steps["step6"] = best_step6

    # Step 7: CC charge 2h
    best_step7 = None
    best_diff7 = float("inf")
    for step in step_list:
        typ = step.get("step_type", "").replace("_", " ").lower()
        if typ == "cc chg":
            duration = parse_duration(step.get("step_time", ""))
            if duration is not None:
                ...
                if diff < best_diff7:
                    ...
                    best_step7 = step
    if best_step7:
        steps["step7"] = best_step7

    # Step 8: Rest 10s (last)
    best_step8 = None
    best_diff8 = float("inf")
    for step in reversed(step_list):
        typ = step.get("step_type", "").replace("_", " ").lower()
        if typ == "rest":
            duration = parse_duration(step.get("step_time", ""))
    ...
    # Step 1
    if "step1" in steps:
        results = check_step_1(steps["step1"], aux)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 2
    if "step2" in steps:
        results = check_step_2(steps["step2"], aux, cycle)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 3
    if "step3" in steps:
        results = check_step_3(steps["step3"], aux)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 4
    if "step4" in steps:
        results = check_step_4(steps["step4"], aux, records)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 6
    if "step6" in steps:
        results = check_step_6(step_list, aux)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 7
    if "step7" in steps:
        results = check_step_7(step_list, aux)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
    ...
    # Step 8
    if "step8" in steps:
        results = check_step_8(step_list, aux)
    else:
        results = [{
            "check": "step_find",
            "result": "NG",
